...
func composeSHA(content string) string {
	...
	// "Exact SHA" is production provenance, not merely a coincidence that all
	// detected image tags match. Require exactly one generated-compose header and
	// exactly one broker provenance value, both agreeing with the complete owned
	// image set. Raw tracked baselines therefore never masquerade as deployed,
	// rollback-safe exact compose state.
	headerRE := regexp.MustCompile(`(?m)^# GENERATED exact-SHA Router VPN production compose: ([0-9a-f]{40})package main
...
func preserveUpdater(target, current string) (string, error) {
	...
	if len(headers) != 1 || headers[0][1] != imageSHA {
		return "unknown"
	}
	brokers := brokerSHARe.FindAllStringSubmatch(content, -1)
	if len(brokers) != 1 || brokers[0][2] != imageSHA {
		return "unknown"
	}
	return imageSHA
}
