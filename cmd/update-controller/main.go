...
func loadController() (*controller, error) {
	...
	if err != nil { return nil, fmt.Errorf("setup token: %w", err) }
...
func (c *controller) require(w http.ResponseWriter, r *http.Request, method string) bool {
	if r.Method != method { w.Header().Set("Allow", method); http.Error(w, "method not allowed", http.StatusMethodNotAllowed); return false }
	if !c.authorized(r) { http.Error(w, "forbidden", http.StatusForbidden); return false }
	return true
}
...
func (c *controller) configured() (bool, string) {
	if _, err := readSecret(c.portainerKeyFile, 16); err != nil { return false, "Portainer API key is not configured server-side" }
	...
	if _, err := hex.DecodeString(strings.ToLower(pin)); err != nil { return false, "Portainer TLS certificate fingerprint is invalid" }
	return true, ""
}
...
func (c *controller) portainerClient() (*http.Client, string, error) {
	...
	if len(pin) != 64 { return nil, "", errors.New("invalid Portainer TLS fingerprint length") }
	if _, err := hex.DecodeString(pin); err != nil { return nil, "", errors.New("invalid Portainer TLS fingerprint") }
	tlsConfig := &tls.Config{
		MinVersion: tls.VersionTLS12,
		// Certificate trust is a local explicit SHA-256 pin. Standard chain
		// verification cannot validate Portainer's default self-signed cert.
		InsecureSkipVerify: true, // verified below by VerifyConnection
		VerifyConnection: func(cs tls.ConnectionState) error {
			if len(cs.PeerCertificates) == 0 { return errors.New("Portainer returned no TLS certificate") }
...
func (c *controller) portainer(method, path string, body any, out any) error {
	...
	if body != nil {
		...
		if len(b) > maxCompose+maxJSON { return errors.New("Portainer request exceeds safety limit") }
		reader = bytes.NewReader(b)
	}
	endpoint := strings.TrimRight(c.portainerURL.String(), "/") + path
	req, err := http.NewRequest(method, endpoint, reader); if err != nil { return err }
	req.Header.Set("X-API-Key", key)
	req.Header.Set("Accept", "application/json")
	if body != nil { req.Header.Set("Content-Type", "application/json") }
	resp, err := client.Do(req); if err != nil { return err }
	defer resp.Body.Close()
	raw, err := io.ReadAll(io.LimitReader(resp.Body, maxCompose+maxJSON+1)); if err != nil { return err }
	if len(raw) > maxCompose+maxJSON { return errors.New("Portainer response exceeds safety limit") }
	if resp.StatusCode/100 != 2 { return fmt.Errorf("Portainer %s %s: HTTP %d: %s", method, path, resp.StatusCode, strings.TrimSpace(string(raw))) }
	if out != nil && len(bytes.TrimSpace(raw)) > 0 { if err := json.Unmarshal(raw, out); err != nil { return fmt.Errorf("decode Portainer response: %w", err) } }
	return nil
}
...
func githubHeaders(req *http.Request) {
	...
	if token := strings.TrimSpace(os.Getenv("GITHUB_TOKEN")); token != "" { req.Header.Set("Authorization", "Bearer "+token) }
}

func githubJSON(endpoint string, out any) error {
	req, err := http.NewRequest(http.MethodGet, endpoint, nil); if err != nil { return err }
	githubHeaders(req)
	client := &http.Client{Timeout: 15 * time.Second, Transport: &http.Transport{Proxy: nil}}
	resp, err := client.Do(req); if err != nil { return err }
	defer resp.Body.Close()
	raw, err := io.ReadAll(io.LimitReader(resp.Body, maxJSON+1)); if err != nil { return err }
	if len(raw) > maxJSON { return errors.New("GitHub response exceeds safety limit") }
	if resp.StatusCode/100 != 2 { return fmt.Errorf("GitHub HTTP %d", resp.StatusCode) }
...
func githubText(endpoint string, limit int64) (string, error) {
	...
	resp, err := client.Do(req); if err != nil { return "", err }
	defer resp.Body.Close()
	raw, err := io.ReadAll(io.LimitReader(resp.Body, limit+1)); if err != nil { return "", err }
	if int64(len(raw)) > limit { return "", errors.New("GitHub file exceeds safety limit") }
	if resp.StatusCode/100 != 2 { return "", fmt.Errorf("GitHub file HTTP %d", resp.StatusCode) }
	return string(raw), nil
}

func (c *controller) workflowSuccess(file, sha string) (bool, error) {
	if !shaRE.MatchString(sha) { return false, errors.New("invalid SHA") }
	...
	if err := githubJSON(endpoint, &runs); err != nil { return false, err }
	for _, run := range runs.Runs {
		if strings.ToLower(run.HeadSHA) == sha && run.HeadBranch == c.branch && run.Status == "completed" && run.Conclusion == "success" { return true, nil }
	}
	return false, nil
}
...
func (c *controller) verifiedTarget(sha string) error {
	...
	if !shaRE.MatchString(sha) { return errors.New("target must be a lowercase full 40-character commit SHA") }
	for _, workflow := range []string{"release-candidate.yml", "publish-arm64-images.yml", "production-release-compose.yml"} {
		...
		if err != nil { return fmt.Errorf("verify %s: %w", workflow, err) }
		if !ok { return fmt.Errorf("%s has no successful exact-SHA %s run", sha, workflow) }
	}
	return nil
}
...
func (c *controller) latestVerified() (string, error) {
	...
	var runs workflowRuns
	if err := githubJSON(endpoint, &runs); err != nil { return "", err }
	seen := map[string]bool{}
	for _, run := range runs.Runs {
		sha := strings.ToLower(run.HeadSHA)
		if seen[sha] || !shaRE.MatchString(sha) || run.HeadBranch != c.branch || run.Status != "completed" || run.Conclusion != "success" { continue }
		seen[sha] = true
		if err := c.verifiedTarget(sha); err == nil { return sha, nil }
	}
...
func validateAndMaterializeTemplate(text, target string) (string, error) {
	if !shaRE.MatchString(target) { return "", errors.New("invalid target SHA") }
	if len(text) == 0 || len(text) > maxCompose { return "", errors.New("production compose baseline size is invalid") }
...
func (c *controller) targetCompose(sha string) (string, error) {
	if err := c.verifiedTarget(sha); err != nil { return "", err }
...
func (c *controller) stackFile(stack stackInfo) (string, error) {
	var payload struct { StackFileContent string `json:"StackFileContent"` }
	if err := c.portainer(http.MethodGet, fmt.Sprintf("/api/stacks/%d/file", stack.ID), nil, &payload); err != nil { return "", err }
	if payload.StackFileContent == "" || len(payload.StackFileContent) > maxCompose { return "", errors.New("Portainer returned an empty/oversized stack file") }
...
func extractUpdaterImage(content string) (string, error) {
	...
	if len(matches) != 1 { return "", fmt.Errorf("expected one updater image in current stack, found %d", len(matches)) }
	return matches[0], nil
}

func preserveUpdater(target, current string) (string, error) {
	oldImage, err := extractUpdaterImage(current); if err != nil { return "", err }
	re := regexp.MustCompile(`ghcr\.io/eabusham2/router-vpn-updater:[0-9a-f]{40}`)
	if len(re.FindAllString(target, -1)) != 1 { return "", errors.New("target compose has invalid updater image count") }
...
func (c *controller) containerState(stack stackInfo, name string) (bool, int, error) {
	var payload struct { State struct { Running bool `json:"Running"`; ExitCode int `json:"ExitCode"`; Status string `json:"Status"` } `json:"State"` }
	path := fmt.Sprintf("/api/endpoints/%d/docker/containers/%s/json", stack.EndpointID, url.PathEscape(name))
	if err := c.portainer(http.MethodGet, path, nil, &payload); err != nil { return false, -1, err }
...
func (c *controller) waitCoreHealthy(stack stackInfo, timeout time.Duration) error {
	...
	for time.Now().Before(deadline) {
		...
		for _, name := range longRunning {
			...
			if err != nil || !running { problems = append(problems, fmt.Sprintf("%s not running (exit=%d)", name, exit)) }
		}
		for _, name := range oneShot {
			running, exit, err := c.containerState(stack, name)
			if err != nil || running || exit != 0 { problems = append(problems, fmt.Sprintf("%s not completed successfully (running=%t exit=%d)", name, running, exit)) }
		}
		if len(problems) == 0 && localHealth("http://127.0.0.1:8786/healthz") && localHealth("http://127.0.0.1:8787/health") { return nil }
...
func (c *controller) status(w http.ResponseWriter, r *http.Request) {
	if !c.require(w, r, http.MethodGet) { return }
	configured, reason := c.configured()
	c.mu.Lock(); state := c.state; busy := state.Status == "applying" || state.Status == "finalizing" || state.Status == "rolling-back"; c.mu.Unlock()
	current := "unknown"
	if configured {
...
func decodeSHARequest(w http.ResponseWriter, r *http.Request) (string, bool) {
	var q struct { SHA string `json:"sha"` }
	dec := json.NewDecoder(http.MaxBytesReader(w, r.Body, 4096))
	if err := dec.Decode(&q); err != nil { http.Error(w, "bad json", http.StatusBadRequest); return "", false }
	return strings.ToLower(strings.TrimSpace(q.SHA)), true
}

func (c *controller) check(w http.ResponseWriter, r *http.Request) {
	if !c.require(w, r, http.MethodPost) { return }
	sha, ok := decodeSHARequest(w, r); if !ok { return }
	var err error
	if sha == "" { sha, err = c.latestVerified() } else { err = c.verifiedTarget(sha) }
...
func (c *controller) applyUpdate(w http.ResponseWriter, r *http.Request) {
	if !c.require(w, r, http.MethodPost) { return }
	sha, ok := decodeSHARequest(w, r); if !ok { return }
	if sha == "" { http.Error(w, "exact target sha is required; use Check latest first", http.StatusBadRequest); return }
...
func (c *controller) reconcileFinalizing() {
	...
	if state.Status != "finalizing" || !shaRE.MatchString(state.TargetSHA) { return }
	configured, _ := c.configured(); if !configured { return }
	stack, err := c.findStack(); if err != nil { return }
...
func main() {
	c, err := loadController(); if err != nil { log.Fatal(err) }
