...
func validReleaseManifestJSON() string {
	sha := strings.Repeat("a", 40); digest := strings.Repeat("b", 64)
...
func TestDecodeReleaseManifestFailsClosed(t *testing.T) {
	...
	if _, err := decodeReleaseManifest(raw); err != nil { t.Fatalf("valid manifest rejected: %v", err) }
	for _, suffix := range []string{` {}`, ` true`, ` null`} { if _, err := decodeReleaseManifest(append(append([]byte(nil), raw...), suffix...)); err == nil { t.Fatalf("trailing release JSON %q was accepted", suffix) } }
	unknown := strings.Replace(validReleaseManifestJSON(), `"schema_version":1`, `"schema_version":1,"unexpected":true`, 1)
	if _, err := decodeReleaseManifest([]byte(unknown)); err == nil { t.Fatal("unknown release manifest field was accepted") }
}
func TestReadSourceManifestRequiresExactRegularFile(t *testing.T) {
	dir := t.TempDir(); path := filepath.Join(dir, "ROUTER-VPN-SOURCE.json"); sha := strings.Repeat("c", 40)
	raw := []byte(`{"repository":"Eabusham2/router-vpn","source_sha":"` + sha + `"}`)
	if err := os.WriteFile(path, raw, 0o600); err != nil { t.Fatal(err) }
	got, err := readSourceManifest(path); if err != nil || got.SourceSHA != sha { t.Fatalf("canonical provenance failed: got=%+v err=%v", got, err) }
	if runtime.GOOS != "windows" { target := filepath.Join(dir, "target.json"); if err := os.Rename(path, target); err != nil { t.Fatal(err) }; if err := os.Symlink(target, path); err != nil { t.Fatal(err) }; if _, err := readSourceManifest(path); err == nil { t.Fatal("symlink provenance was accepted") } }
}
