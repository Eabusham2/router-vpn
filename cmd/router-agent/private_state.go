	...
	validateAncestors := func() error {
		for current := parent; ; current = filepath.Dir(current) {
			...
			if err != nil {
				...
				return fmt.Errorf("refusing non-directory/symlink privileged state path component %s", current)
			}
...
func atomicWritePrivilegedStateTargetUnchanged(path string, before os.FileInfo) error {
	current, err := os.Lstat(path)
	if before == nil {
		if err == nil {
			return fmt.Errorf("privileged state %s appeared before adoption", path)
		}
		if os.IsNotExist(err) {
			return nil
		}
	...
	if err != nil {
		if os.IsNotExist(err) {
			return fmt.Errorf("privileged state %s disappeared before adoption", path)
		}
		return err
	}
	if current.Mode()&os.ModeSymlink != 0 || !current.Mode().IsRegular() || !os.SameFile(before, current) {
		return fmt.Errorf("privileged state %s identity changed before adoption", path)
	}
	return nil
}
...
func atomicWritePrivilegedState(path string, body []byte) error {
	...
	if err := atomicWritePrivilegedStateTargetUnchanged(path, targetBefore); err != nil {
		return err
	}
