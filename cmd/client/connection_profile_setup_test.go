...
func postSetup(t *testing.T, a *app, path string, body map[string]any) *httptest.ResponseRecorder {
	...
	switch path {
	case "/api/connection-profile/setup/save":
		a.saveConnectionProfileSetup(rr, req)
	case "/api/connection-profile/setup/update":
		a.updateConnectionProfileSetup(rr, req)
	case "/api/connection-profile/setup/load":
		a.loadConnectionProfileSetup(rr, req)
	case "/api/connection-profile/setup/delete":
		a.deleteConnectionProfileSetup(rr, req)
	default:
...
func TestConnectionProfileSetupStoresExactHopGraphAndNoSecrets(t *testing.T) {
	...
	if rr.Code != http.StatusOK {
		t.Fatalf("save setup status=%d body=%s", rr.Code, rr.Body.String())
	}
	...
	if !validProfileID(id) {
		t.Fatalf("invalid returned setup id: %#v", response)
	}
	...
	if got.Mode != "custom:stealth" || got.Prefs == nil {
		t.Fatalf("unexpected profile=%#v", got)
	}
	if !got.Prefs.MultihopEnabled || got.Prefs.MultihopEntryID != "entry" || got.Prefs.MultihopExitID != "exit" {
		t.Fatalf("exact multihop graph not stored: %#v", got.Prefs)
	}
	if strings.Join(got.Prefs.CustomLayers, ",") != "reality,wireguard" {
		t.Fatalf("custom layers=%#v", got.Prefs.CustomLayers)
	}

	meta, err := loadConnectionProfileSetupMeta(a)
	if err != nil {
		t.Fatal(err)
	}
	if meta.Entries[id].MultihopExitMode != "hysteria2" {
		t.Fatalf("setup meta=%#v", meta)
	}
...
func TestConnectionProfileSetupRejectsBadOrExternalHopGraph(t *testing.T) {
	...
	for _, body := range cases {
		...
		if rr.Code >= 200 && rr.Code < 300 {
			t.Fatalf("bad graph unexpectedly accepted: %#v -> %s", body, rr.Body.String())
		}
	}
}
...
func TestConnectionProfileSetupLoadReturnsExactExitTransport(t *testing.T) {
	...
	if err := json.Unmarshal(saved.Body.Bytes(), &savePayload); err != nil {
		t.Fatal(err)
	}
	...
	if payload["multihop_exit_mode"] != "hysteria2" || payload["multihop_entry_id"] != "entry" || payload["multihop_exit_id"] != "exit" {
		t.Fatalf("load lost setup: %#v", payload)
	}
	if payload["mode"] != "smart-auto" || payload["selected_node_id"] != "home" {
		t.Fatalf("load lost node/mode: %#v", payload)
	}
}

func TestConnectionProfileSetupLoadPreflightsMetadataBeforeMutatingSelection(t *testing.T) {
	a := setupProfileTestApp(t)
	saved := postSetup(t, a, "/api/connection-profile/setup/save", map[string]any{
		"name": "Home snapshot", "mode": "smart-auto", "multihop_enabled": false,
	})
	if saved.Code != http.StatusOK {
	...
	id := payload["profile"].(map[string]any)["id"].(string)

	a.mu.Lock()
	a.profiles.SelectedID = "entry"
	a.state.RouterID = "entry"
	a.mu.Unlock()
	if err := os.WriteFile(connectionProfileSetupMetaPath(a), []byte("{not-json\n"), 0o600); err != nil {
		t.Fatal(err)
	}

	loaded := postSetup(t, a, "/api/connection-profile/setup/load", map[string]any{"id": id})
	if loaded.Code < 400 {
		t.Fatalf("corrupt setup metadata unexpectedly loaded: %d %s", loaded.Code, loaded.Body.String())
	}
	a.mu.Lock()
	selected := a.profiles.SelectedID
	routerID := a.state.RouterID
	a.mu.Unlock()
	if selected != "entry" || routerID != "entry" {
