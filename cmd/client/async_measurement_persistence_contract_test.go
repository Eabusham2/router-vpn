...
func TestAsyncMeasurementPersistenceRequiresFreshIdentityAndSession(t *testing.T) {
    ...
    for _, marker := range []string{
        ...
        "VPN session changed while public-exit lookup was running",
        "active VPN path changed while public-exit lookup was running",
        "VPN session changed while DNS Retest was running",
        ...
        if !strings.Contains(text, marker) { t.Fatalf("async measurement persistence contract missing %q", marker) }
    }
