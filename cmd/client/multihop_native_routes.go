...
func (a *app) nativeMultihopConnect(w http.ResponseWriter, r *http.Request) {
	...
	if control.NodeKind == "external" { http.Error(w, "selected node is external; use External Connect for direct or Router VPN-entry -> external-exit chaining", http.StatusBadRequest); return }
	...
	a.mu.Lock();if a.cmd!=cmd{a.mu.Unlock();http.Error(w,"multihop runtime changed during exit proof",http.StatusConflict);return};a.state.Connected=true;a.state.Phase="connected";a.state.LastError="";now:=time.Now().UTC().Format(time.RFC3339);for i:=range a.profiles.Profiles{if a.profiles.Profiles[i].ID==sel.Entry.ID||a.profiles.Profiles[i].ID==sel.Exit.ID{a.profiles.Profiles[i].UseCount++;a.profiles.Profiles[i].LastUsedAt=now}};persistErr:=a.persistProfilesLocked();a.mu.Unlock()
