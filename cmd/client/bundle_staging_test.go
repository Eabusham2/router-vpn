...
func TestSafeBundleTokenRejectsTraversalSeparatorsAndDriveSyntax(t *testing.T) {
	...
	for _, value := range valid {
		if !safeBundleToken(value) { t.Fatalf("valid bundle token rejected: %q", value) }
	}
	invalid := []string{"", ".", "..", "../wg", "a/b", `a\b`, "C:x", "C:\\x", "nul\x00x", strings.Repeat("a", maxBundleNameBytes+1)}
	for _, value := range invalid {
		if safeBundleToken(value) { t.Fatalf("unsafe bundle token accepted: %q", value) }
	}
}
...
func TestBundleStagingFailureLeavesNoGeneratedProfile(t *testing.T) {
	...
	if err := stage.writeProfiles(profiles); err == nil { t.Fatal("unsafe bundle filename was accepted") }
	...
	if err != nil { t.Fatal(err) }
	if len(entries) != 0 { t.Fatalf("failed import left staging directories: %v", entries) }
}
...
func TestBundleStagingCommitsAtomicallyWithPrivateModes(t *testing.T) {
	...
	if err := stage.writeProfiles(profiles); err != nil { t.Fatal(err) }
	if _, err := os.Stat(filepath.Join(root, "generated", "router-test")); !os.IsNotExist(err) {
		t.Fatalf("profile became visible before commit: %v", err)
	}
	if err := stage.commit(root, "router-test"); err != nil { t.Fatal(err) }
	final := filepath.Join(root, "generated", "router-test", "wg", "wg.conf")
	st, err := os.Stat(final)
	if err != nil { t.Fatal(err) }
	if st.Mode().Perm()&0o077 != 0 { t.Fatalf("staged private profile permissions too broad: %o", st.Mode().Perm()) }
	stage.cleanup()
	entries, err := os.ReadDir(filepath.Join(root, ".bundle-staging"))
	if err != nil { t.Fatal(err) }
	if len(entries) != 0 { t.Fatalf("successful import left staging directories: %v", entries) }
}
