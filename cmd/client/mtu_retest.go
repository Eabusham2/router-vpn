...
func (a *app) retestMTU(w http.ResponseWriter, r *http.Request) {
	...
	if strings.EqualFold(strings.TrimSpace(p.NodeKind), "external") || p.External != nil {
		http.Error(w, "MTU retest currently requires a connected Router VPN node with private path proof", http.StatusConflict)
		return
	}
	...
	a.mu.Unlock()
	if !st.Connected {
		http.Error(w, "connect the selected Router VPN node first; MTU Retest benchmarks only the proven private tunnel path", http.StatusConflict)
		return
	}
	if st.Mode == "multihop" {
		http.Error(w, "disconnect multihop and retest each single-hop path separately; one MTU result must not be mislabeled as both hops", http.StatusConflict)
		return
	}
	if strings.ToLower(strings.TrimSpace(p.MTUPolicy)) != "auto" {
		http.Error(w, "set this node MTU policy to Auto before Retest", http.StatusConflict)
		return
	}
	...
	if mode == "" || mode == "off" {
		http.Error(w, "connected runtime mode is unknown; refusing an unkeyed MTU retest", http.StatusConflict)
		return
	}
	...
	if runErr != nil {
		...
		http.Error(w, "MTU Retest failed closed: "+detail, http.StatusBadGateway)
		return
	}
	...
	updated, reloadErr := reloadMTUProfileStore(a)
	if reloadErr != nil {
		http.Error(w, "MTU Retest completed but the updated profile could not be reloaded: "+reloadErr.Error(), http.StatusInternalServerError)
		return
	}
	...
	_ = json.NewEncoder(w).Encode(map[string]any{
		...
		"result": result,
		"note": "bounded private-node loss/RTT/throughput comparison; the result is cached by network/path context and does not claim MTU caused any earlier cellular regression",
	})
}
...
func reloadMTUProfileStore(a *app) (common.RouterProfile, error) {
	raw, err := os.ReadFile(a.cfg.ProfilesFile)
	if err != nil {
	...
	if store.SelectedID == "" {
		return common.RouterProfile{}, errors.New("updated router profile store has no selected node")
	}
	...
	if !found {
		return common.RouterProfile{}, fmt.Errorf("updated selected router %q is missing", store.SelectedID)
	}
	a.mu.Lock()
	// The tuner changes only MTU measurement fields in routers.json. Preserve the
	// live route/session state while refreshing the persistent profile snapshot.
	a.profiles = store
	a.state.RouterID = store.SelectedID
	a.mu.Unlock()
	return selected, nil
}
