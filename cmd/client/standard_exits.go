...
func newStandardExitID() string {
	b := make([]byte, 6)
	if _, err := rand.Read(b); err != nil { return "exit-local" }
...
func validateCIDRs(values []string, label string, required bool) error {
	...
	for _, raw := range values {
		if _, _, err := net.ParseCIDR(strings.TrimSpace(raw)); err != nil { return fmt.Errorf("invalid %s %q", label, raw) }
	}
	return nil
}

func validateStandardExit(e *standardExit) error {
	if e == nil { return errors.New("standard exit is required") }
	e.ID = strings.TrimSpace(e.ID)
	if e.ID == "" { e.ID = newStandardExitID() }
	if !validProfileID(e.ID) { return errors.New("invalid standard exit id") }
	e.Name = strings.TrimSpace(e.Name)
	if e.Name == "" { e.Name = "Custom Exit" }
	if len(e.Name) > 120 { return errors.New("standard exit name is too long") }
	...
	if e.Protocol == "openvpn" {
		if err := normalizeOpenVPNStandardExit(e); err != nil { return err }
	}
	server, err := normalizeEndpoint(e.Server)
	if err != nil { return fmt.Errorf("standard exit server: %w", err) }
	e.Server = server
	if e.ServerPort < 1 || e.ServerPort > 65535 { return errors.New("standard exit server port must be 1..65535") }
	ip := net.ParseIP(strings.TrimSpace(e.ExpectedPublicIP))
	if ip == nil || ip.IsPrivate() || ip.IsLoopback() || ip.IsLinkLocalUnicast() || ip.IsUnspecified() { return errors.New("expected_public_ip must be the public address expected after traffic really exits through this custom exit") }
	...
	for label, value := range map[string]string{"username": e.Username, "password": e.Password, "secret": e.Secret, "tls_server_name": e.TLSServerName, "wg_private_key": e.WGPrivateKey, "wg_peer_public_key": e.WGPeerPublicKey, "wg_pre_shared_key": e.WGPreSharedKey} {
		if len(value) > 4096 { return fmt.Errorf("%s is too long", label) }
	}
	switch e.Protocol {
	case "socks5":
		if (e.Username == "") != (e.Password == "") { return errors.New("SOCKS5 username/password must either both be set or both be empty") }
	case "shadowsocks":
		...
		if !allowed[e.Method] { return errors.New("unsupported or insecure Shadowsocks method") }
		if strings.TrimSpace(e.Secret) == "" { return errors.New("Shadowsocks password/PSK is required") }
	case "hysteria2":
		if strings.TrimSpace(e.Secret) == "" { return errors.New("Hysteria2 password is required") }
		e.TLSServerName = strings.TrimSpace(e.TLSServerName)
		if e.TLSServerName == "" || strings.ContainsAny(e.TLSServerName, " /\\?#@") { return errors.New("Hysteria2 requires a valid TLS server name") }
	case "wireguard":
		...
		if e.WGMTU != 0 && (e.WGMTU < 1280 || e.WGMTU > 9000) { return errors.New("WireGuard MTU must be 1280..9000 when specified") }
	case "openvpn":
...
func loadStandardExitStore() (standardExitStore, error) {
	...
	if errors.Is(err, os.ErrNotExist) { return standardExitStore{SchemaVersion: standardExitStoreVersion, Exits: []standardExit{}}, nil }
	if err != nil { return standardExitStore{}, err }
	if info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() { return standardExitStore{}, errors.New("standard-exits.json must be a regular non-symlink file") }
	if info.Size() > standardExitMaxStore { return standardExitStore{}, errors.New("standard-exits.json exceeds size limit") }
	if runtime.GOOS != "windows" && info.Mode().Perm()&0o077 != 0 { return standardExitStore{}, errors.New("standard-exits.json permissions are too broad; expected 0600") }
	...
	if store.SchemaVersion != standardExitStoreVersion { return standardExitStore{}, fmt.Errorf("unsupported standard exit store schema %d", store.SchemaVersion) }
...
func persistStandardExitStore(store standardExitStore) error {
	...
	for i := range store.Exits { if err := validateStandardExit(&store.Exits[i]); err != nil { return err } }
	path := standardExitStorePath(); dir := filepath.Dir(path)
	if err := os.MkdirAll(dir, 0o700); err != nil { return err }
	if info, err := os.Lstat(path); err == nil && info.Mode()&os.ModeSymlink != 0 { return errors.New("refusing to replace symlink standard-exits.json") } else if err != nil && !errors.Is(err, os.ErrNotExist) { return err }
...
func standardExitRuntimeParts(e standardExit, detour string) (map[string]any, map[string]any, error) {
	...
	if e.Protocol == "openvpn" { return nil, nil, errors.New("OpenVPN standard exits use the native OpenVPN adapter, not the sing-box graph") }
	...
	switch e.Protocol {
	case "socks5":
		out["type"] = "socks"; out["version"] = "5"; if e.Username != "" { out["username"] = e.Username; out["password"] = e.Password }
	case "shadowsocks":
...
func standardExitSaveHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(http.MaxBytesReader(w, r.Body, 320<<10)).Decode(&candidate); err != nil { http.Error(w, "bad json", http.StatusBadRequest); return }
