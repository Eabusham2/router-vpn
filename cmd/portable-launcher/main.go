...
func main() {
	...
	root := filepath.Dir(exe); appDir := filepath.Join(root,"App","RouterVPN"); dataDir := filepath.Join(root,"Data"); generatedDir := filepath.Join(dataDir,"generated"); modesDir := filepath.Join(appDir,"modes"); dataModes := filepath.Join(dataDir,"modes.windows.json"); dataLogical := filepath.Join(dataDir,"logical-modes.json"); nativeApp := filepath.Join(appDir,"client","RouterVPN-Windows-App.ps1")
	for _,dir:=range []string{dataDir,generatedDir}{if err:=os.MkdirAll(dir,0o700);err!=nil{fatal(err)}}
	for _,required:=range []string{filepath.Join(appDir,"router-vpn-client.exe"),filepath.Join(appDir,"modes.json"),filepath.Join(appDir,"logical-modes.json"),modesDir,nativeApp}{if _,err:=os.Stat(required);err!=nil{fatal(fmt.Errorf("portable package is incomplete: %s: %w",required,err))}}
...
func openNativeApp(script string)(*exec.Cmd,error){if !fileExists(script){return nil,fmt.Errorf("native Windows WPF app missing: %s",script)};cmd:=exec.Command("powershell.exe","-NoProfile","-ExecutionPolicy","Bypass","-File",script,"-BaseUrl","http://127.0.0.1:8788");cmd.Stdout=os.Stdout;cmd.Stderr=os.Stderr;if err:=cmd.Start();err!=nil{return nil,fmt.Errorf("start native Windows app: %w",err)};return cmd,nil}
func stopPortableController(cmd *exec.Cmd){client:=&http.Client{Timeout:2*time.Second};req,_:=http.NewRequest(http.MethodPost,localURL+"api/emergency-stop",bytes.NewReader(nil));req.Header.Set("content-type","application/json");if resp,err:=client.Do(req);err==nil{_,_=io.Copy(io.Discard,resp.Body);_=resp.Body.Close()};if cmd!=nil&&cmd.Process!=nil{_=cmd.Process.Kill();_,_=cmd.Process.Wait()}}
