...
# Saved values are loaded first, but an explicitly supplied environment value
# still wins. This makes custom listener-port/host choices survive Merlin
# firewall reloads and router reboots instead of applying only once at install.
[ ! -f "$CONFIG" ] || . "$CONFIG"

DST=${ROUTER_VPN_HOST:-${SAVED_ROUTER_VPN_HOST:-192.168.50.133}}
WG_PORT=${WG_PORT:-${SAVED_WG_PORT:-51820}}
AWG_PORT=${AWG_PORT:-${SAVED_AWG_PORT:-585}}
ROSENPASS_PORT=${ROSENPASS_PORT:-${SAVED_ROSENPASS_PORT:-51822}}
REALITY_PORT=${REALITY_PORT:-${SAVED_REALITY_PORT:-443}}
HY2_PORT=${HY2_PORT:-${SAVED_HY2_PORT:-8443}}
SS_PORT=${SS_PORT:-${SAVED_SS_PORT:-8388}}
XRAY_PQ_PORT=${XRAY_PQ_PORT:-${SAVED_XRAY_PQ_PORT:-10443}}
XHTTP_PORT=${XHTTP_PORT:-${SAVED_XHTTP_PORT:-11443}}
SS_V2RAY_PORT=${SS_V2RAY_PORT:-${SAVED_SS_V2RAY_PORT:-12443}}
NAIVE_PORT=${NAIVE_PORT:-${SAVED_NAIVE_PORT:-13443}}
ACME_EXTERNAL_PORT=${ACME_EXTERNAL_PORT:-${SAVED_ACME_EXTERNAL_PORT:-80}}
ACME_INTERNAL_PORT=${ACME_INTERNAL_PORT:-${SAVED_ACME_INTERNAL_PORT:-18080}}
ROUTER_VPN_WAN_INTERFACE=${ROUTER_VPN_WAN_INTERFACE:-${SAVED_ROUTER_VPN_WAN_INTERFACE:-}}
...
validate_port(){
  NAME=$1; VALUE=$2
  case "$VALUE" in
    ''|*[!0-9]*) fail "$NAME must be a numeric TCP/UDP port." ;;
  esac
...
validate_settings(){
  case "$DST" in
    ''|*[!A-Za-z0-9._:-]*) fail 'ROUTER_VPN_HOST contains unsupported characters.' ;;
  esac
  if [ -n "$ROUTER_VPN_WAN_INTERFACE" ]; then
    case "$ROUTER_VPN_WAN_INTERFACE" in
      *[!A-Za-z0-9._:-]*) fail 'ROUTER_VPN_WAN_INTERFACE contains unsupported characters.' ;;
    esac
  fi
  validate_port WG_PORT "$WG_PORT"
  validate_port AWG_PORT "$AWG_PORT"
  validate_port ROSENPASS_PORT "$ROSENPASS_PORT"
  validate_port REALITY_PORT "$REALITY_PORT"
  validate_port HY2_PORT "$HY2_PORT"
  validate_port SS_PORT "$SS_PORT"
  validate_port XRAY_PQ_PORT "$XRAY_PQ_PORT"
...
write_config(){
  validate_settings
  cat > "$CONFIG" <<EOF
# Generated by router-vpn. Public forwarding settings only; no VPN keys/tokens.
SAVED_ROUTER_VPN_HOST='$DST'
SAVED_ROUTER_VPN_WAN_INTERFACE='$ROUTER_VPN_WAN_INTERFACE'
SAVED_WG_PORT='$WG_PORT'
SAVED_AWG_PORT='$AWG_PORT'
SAVED_ROSENPASS_PORT='$ROSENPASS_PORT'
SAVED_REALITY_PORT='$REALITY_PORT'
SAVED_HY2_PORT='$HY2_PORT'
SAVED_SS_PORT='$SS_PORT'
SAVED_XRAY_PQ_PORT='$XRAY_PQ_PORT'
SAVED_XHTTP_PORT='$XHTTP_PORT'
SAVED_SS_V2RAY_PORT='$SS_V2RAY_PORT'
SAVED_NAIVE_PORT='$NAIVE_PORT'
SAVED_ACME_EXTERNAL_PORT='$ACME_EXTERNAL_PORT'
SAVED_ACME_INTERNAL_PORT='$ACME_INTERNAL_PORT'
EOF
  chmod 600 "$CONFIG"
}
...
remove(){
  ...
  rm -f "$RUNTIME" "$CONFIG"
  say 'Router VPN forward hooks/config removed. Other JFFS script content was preserved.'
}
