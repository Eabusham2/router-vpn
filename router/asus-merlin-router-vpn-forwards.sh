...
# The persisted file uses shell default assignments (: "${VAR:=value}").
# That means saved settings survive reboot/firewall reload, while an explicit
# environment override supplied for a later install still wins.
[ ! -f "$CONFIG" ] || . "$CONFIG"
