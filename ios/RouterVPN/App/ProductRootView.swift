...
private let routerVPNProductParityContract = "map-first swipe-up Connect Disconnect quick kill switch Multihop Settings Mode DNS Speed Lab idle loaded download upload latency Auto Custom min max time SMART AUTO default AUTO all logical presets CUSTOM preset builder saved delete Router node Custom external typed external node builder WireGuard SOCKS5 Shadowsocks Hysteria2 color-coded hops real coordinates Home Fastest Custom DoT DoH DoH3 Rescue actual public VPN exit selected-node proof opt-in real user location no IP geolocation crash-recoverable temporary Speed Lab"
