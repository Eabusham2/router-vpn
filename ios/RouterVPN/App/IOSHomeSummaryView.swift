...
struct IOSHomeSummaryView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                ...
                Spacer()
                Text(model.connected ? "Connected" : "Off")
                    .font(.caption.bold())
            ...
            Text("Warnings: \(warnings.isEmpty ? "None" : warnings.joined(separator: " | "))")
                .foregroundStyle(warnings.isEmpty ? .secondary : .orange)
            if !exitError.isEmpty { Text(exitError).foregroundStyle(.red) }
            HStack {
                Button(provingExit ? "Proving…" : "Prove actual exit") { Task { await proveActualExit() } }
                    .disabled(!model.connected || provingExit)
                Button("Emergency Disconnect") {
                    actualExitIP = ""
        ...
        .onChange(of: model.connected) { _, connected in
            if !connected { actualExitIP = ""; actualExitStatus = "not-connected"; exitError = "" }
        }
    ...
    private func clearExitProofForRuntimeChange() {
        if actualExitStatus == "proved" { actualExitIP = ""; actualExitStatus = model.connected ? "unproven" : "not-connected" }
    }
}

// Apple Home contract: node location public endpoint actual public VPN exit
...
// Emergency Disconnect. Cached profile.publicIP is never used as live proof.
