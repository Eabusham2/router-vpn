...
final class NativeWireGuardController implements Tunnel {
    interface Callback {
        void done(State state, String message, Throwable error);
    }
    ...
    void disconnect(Callback callback) {
        executor.execute(() -> {
            try {
                ...
                callback.done(state, "WireGuard disconnect failed: " + safeMessage(error), error);
            }
        });
    }
    ...
    private static Config loadWireGuardConfig(File privateBundle) throws Exception {
        if (!privateBundle.isFile()) {
            throw new IllegalStateException("Import/link a Router VPN node first.");
        }
