...
public final class MainActivity extends Activity {
    ...
    private View buildUi() {
        ...
        statusView = text("Import the private router-vpn-bundle.json generated by your home router.", 16, false);
    ...
    private String onboardingText(int step) {
        switch (step) {
            ...
            case 1:
                return "Deploy the home node in Portainer: Stacks → Add stack → Repository. Compose path: server/portainer-current.yaml. Normal environment values: WAN_INTERFACE=eth0, LAN_CIDR=192.168.50.0/24, ADGUARD4=192.168.50.133. ENDPOINT is optional and can stay blank for auto-detection. The production server stack uses exact published images.";
            case 2:
            ...
            case 3:
                return "On the home LAN open http://192.168.50.133:8786/. Downloads are on demand: matching GitHub client artifact first, then a router-local build of only the requested client package if unavailable. The private router-vpn-bundle.json and ASUS helper are available from the Setup Center; temporary package files are deleted after delivery.";
            case 4:
                return "ASUS Merlin WAN forwarding: use the Setup Center helper over SSH. It preserves existing nat-start/firewall-start content. External TCP 80 maps to AI Board TCP 18080; 443/TCP, 585/UDP, 8388/TCP+UDP, 8443/UDP, 10443/11443/12443 TCP, 13443 TCP+UDP, 14443/TCP, 15443/TCP+UDP, and 51820/51822 UDP map to the AI Board. Never expose 1080, 8786, 8787, 14444, 9443, SSH, Portainer, or AdGuard admin. TCP 8443 ASUS management does not conflict with Router VPN UDP 8443.";
            case 5:
            ...
            case 6:
                return "Connection choices: AUTO / SMART AUTO / CUSTOM plus logical manual modes and ALL. Compatible logical modes use a WireGuard/AmneziaWG base selector with honest fallback reporting. DNS choices in the full client include fastest real DNS query RTT measured at the home exit, Home AdGuard, custom UDP/TCP, DoT, DoH, DoH3, and DNS Rescue.";
            case 7:
    ...
    private void validateBundle(JSONObject bundle) {
        if (!bundle.has("profiles") || !bundle.has("modes")) throw new IllegalArgumentException("This is not a Router VPN client bundle");
    }
