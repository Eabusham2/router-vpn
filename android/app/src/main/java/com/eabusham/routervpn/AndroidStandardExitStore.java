...
final class AndroidStandardExitStore {
    ...
    static void validate(Entry e) throws Exception {
        ...
        if(!Arrays.asList("wireguard","socks5","shadowsocks","hysteria2").contains(e.protocol))throw new IllegalArgumentException("Unsupported custom exit protocol: "+e.protocol);
        ...
        if("socks5".equals(e.protocol)&&e.username.isEmpty()!=e.password.isEmpty())throw new IllegalArgumentException("SOCKS5 username/password must both be set or both be empty.");
    ...
    private static String normalizeProtocol(String p){p=p==null?"":p.trim().toLowerCase(Locale.ROOT);if("wg".equals(p))return"wireguard";if("socks".equals(p))return"socks5";if("ss".equals(p))return"shadowsocks";if("hy2".equals(p))return"hysteria2";if("ovpn".equals(p))return"openvpn";return p;}
