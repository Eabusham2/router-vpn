...
final class AndroidConnectionProfilesDialog {
    ...
    private static final class Controller {
        ...
        void show(){
            ...
            boolean busy=AndroidVpnMutationGuard.isBusy(activity);name.setEnabled(!busy);profiles.setEnabled(!busy);add.setEnabled(!busy);load.setEnabled(!busy);update.setEnabled(!busy);delete.setEnabled(!busy);
            ...
            refresh();if(busy)status.setText("Disconnect Router VPN or let the active transition finish before Add / Load / Update / Delete. Refresh remains available.");new AlertDialog.Builder(activity).setTitle("Connection profiles").setView(body).setPositiveButton("Close",null).show();
        }
        ...
        void write(boolean updating){try{if(AndroidVpnMutationGuard.isBusy(activity))throw new IllegalStateException("Disconnect Router VPN or let the active transition finish before changing connection profiles.");String clean=name.getText().toString().trim();AndroidConnectionProfileStore.Record result;if(updating){AndroidConnectionProfileStore.Record current=selected();if(current==null)throw new IllegalStateException("Select a saved profile first.");result=store.update(current.id,clean);}else result=store.add(clean);status.setText((updating?"Updated ":"Added ")+result.name+" • "+result.mode);refresh();if(onChanged!=null)onChanged.run();}catch(Throwable error){status.setText((updating?"Update":"Add")+" failed: "+safe(error));}}
        void load(){try{if(AndroidVpnMutationGuard.isBusy(activity))throw new IllegalStateException("Disconnect Router VPN or let the active transition finish before loading a connection profile.");AndroidConnectionProfileStore.Record current=selected();if(current==null)throw new IllegalStateException("Select a saved profile first.");AndroidConnectionProfileStore.Record result=store.load(current.id);status.setText("Loaded "+result.name+" • "+result.mode+". Connect separately to prove the path.");if(onChanged!=null)onChanged.run();}catch(Throwable error){status.setText("Load failed: "+safe(error));}}
        void delete(){try{if(AndroidVpnMutationGuard.isBusy(activity))throw new IllegalStateException("Disconnect Router VPN or let the active transition finish before deleting a connection profile.");AndroidConnectionProfileStore.Record current=selected();if(current==null)throw new IllegalStateException("Select a saved profile first.");store.delete(current.id);status.setText("Deleted "+current.name+".");refresh();if(onChanged!=null)onChanged.run();}catch(Throwable error){status.setText("Delete failed: "+safe(error));}}
