...
final class AndroidConnectionProfilesDialog {
    ...
    private static final class Controller {
        ...
        void show(){
            ...
            TextView note=new TextView(activity);note.setText("Save/load the selected Router or Custom node plus current Mode/CUSTOM layers, DNS, kill switch, IPv6, MTU and multihop choices. Linked node keys, API tokens and external credentials are never copied into these profiles. Load restores choices only; Connect still has to establish and prove the real VPN path.");body.addView(note);
            ...
            refresh();new AlertDialog.Builder(activity).setTitle("Connection profiles").setView(body).setPositiveButton("Close",null).show();
        }
