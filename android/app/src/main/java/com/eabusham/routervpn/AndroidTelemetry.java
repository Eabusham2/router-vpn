...
final class AndroidTelemetry {
    ...
    private PathResult probePrivatePath(int samples)throws Exception{
        ...
        if(values.isEmpty())throw new IllegalStateException("Current private tunnel path did not answer.");Collections.sort(values);AndroidHomeStateStore.Snapshot state=AndroidHomeStateStore.snapshot(context);String proof="multihop".equals(state.logicalMode)?"HTTP RTT to active multihop exit private Router API through Android's current VPN path":"HTTP RTT to active session node private Router API through Android's current VPN path";return new PathResult(round(percentile(values,.5)),values.size(),failed,proof);
    }
    ...
    private JSONObject activeBundle()throws Exception{
        AndroidHomeStateStore.Snapshot state=AndroidHomeStateStore.snapshot(context);
        String id;
        if(state.connected){
            if("multihop".equals(state.logicalMode)){
                if(state.activeExitId==null||state.activeExitId.isEmpty())throw new IllegalStateException("Active multihop exit identity is unavailable; refusing to guess current-path telemetry.");
                requireMultihopEngineUp();
                id=state.activeExitId;
            }else{
                if(state.activeNodeId==null||state.activeNodeId.isEmpty())throw new IllegalStateException("Active session node identity is unavailable; reconnect before running current-path telemetry.");
