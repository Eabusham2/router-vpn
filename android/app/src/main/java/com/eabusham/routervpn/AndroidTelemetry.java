...
final class AndroidTelemetry {
    ...
    private PathResult probePrivatePath(int samples)throws Exception{
        ...
        if(values.isEmpty())throw new IllegalStateException("Current private tunnel path did not answer.");Collections.sort(values);AndroidHomeStateStore.Snapshot state=AndroidHomeStateStore.snapshot(context);String proof="multihop".equals(state.logicalMode)?"HTTP RTT to active multihop exit private Router API through Android's current VPN path":"HTTP RTT to selected node private Router API through Android's current VPN path";return new PathResult(round(percentile(values,.5)),values.size(),failed,proof);
    }
    ...
    private JSONObject activeBundle()throws Exception{
        ...
        String id;
        if(state.connected&&"multihop".equals(state.logicalMode)){
