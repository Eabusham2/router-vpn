...
final class AndroidProfileSettingsDialog {
    static void show(Activity activity, AndroidNodeStore store, Runnable onSaved) {
        try {
            ...
            Spinner startLayer=spinner(activity,new String[]{"Off — default","AES-256-GCM — authenticated Android Libbox modes","AES-256-GCM + XOR whitening — authenticated AES plus protected obfuscation relay"},index(startLayerValues,profile.optString("start_layer",AndroidStartLayer.OFF))); body.addView(label(activity,"Start encryption / obfuscation layer")); body.addView(startLayer);
            TextView startLayerNote=label(activity,"AES uses vetted Shadowsocks 2022 BLAKE3 AES-256-GCM and is composed only into proved Android Libbox raw modes (Shadowsocks, Hysteria2, Naive H2/H3). AES+XOR keeps AES-256-GCM as the authenticated security boundary and adds only reversible whitening through the VpnService-owned protected local relay. XOR is never counted as encryption. Unsupported raw modes still fail closed."); body.addView(startLayerNote);
