...
final class AndroidProductParity {
    ...
    private static JSONObject benchmarkDNS(JSONObject bundle,JSONObject profile)throws Exception{
        ...
        try(Socket socket=new Socket()){socket.connect(new InetSocketAddress(address,port),8000);socket.setSoTimeout(45000);OutputStream out=socket.getOutputStream();String request="POST /api/dns/benchmark HTTP/1.1\r\nHost: "+host+"\r\nAuthorization: Bearer "+token+"\r\nContent-Type: application/json\r\nContent-Length: 2\r\nConnection: close\r\n\r\n{}";out.write(request.getBytes(StandardCharsets.US_ASCII));out.flush();byte[] response=readLimited(socket.getInputStream(),MAX_HTTP);String text=new String(response,StandardCharsets.UTF_8);int split=text.indexOf("\r\n\r\n");if(split<0)throw new IllegalStateException("DNS benchmark returned an invalid HTTP response.");String headers=text.substring(0,split);if(!(headers.startsWith("HTTP/1.1 200 ")||headers.startsWith("HTTP/1.0 200 ")))throw new IllegalStateException("DNS benchmark failed: "+headers.split("\r\n",2)[0]);String body=text.substring(split+4).trim();return new JSONObject(body);}
    }

    private static void persistBenchmark(AndroidNodeStore store,JSONObject bundle,JSONObject result)throws Exception{JSONObject profile=selectedProfile(bundle);if(profile==null)return;JSONArray results=result.optJSONArray("results");JSONObject winner=result.optJSONObject("winner");if(results!=null)profile.put("dns_results",results);if(winner!=null&&!winner.optString("address","").isEmpty()){profile.put("fastest_dns_host",winner.optString("address"));profile.put("fastest_dns_name",winner.optString("name"));profile.put("fastest_dns_latency_ms",winner.optDouble("latency_ms",0));if("fastest".equalsIgnoreCase(profile.optString("dns_mode","")))profile.put("dns_host",winner.optString("address"));}store.importBundle(bundle.toString().getBytes(StandardCharsets.UTF_8));}
