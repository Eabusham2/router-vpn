...
final class AndroidConnectionProfileStore {
    ...
    static final class Record {
        final String id,name,nodeKind,nodeId,mode; final List<String> customLayers;
        Record(String id,String name,String nodeKind,String nodeId,String mode,List<String>layers){this.id=id;this.name=name;this.nodeKind=nodeKind;this.nodeId=nodeId;this.mode=mode;this.customLayers=layers;}
        @Override public String toString(){return name+" • "+mode+" • "+("external".equals(nodeKind)?"Custom":"Router");}
    }
    ...
    synchronized Record load(String id) throws Exception {
        requireIdle("loading a connection profile");JSONObject row=find(id);String kind=row.getString("node_kind"),nodeId=row.getString("node_id");
        if("router-vpn".equals(kind)){
            ...
            nodes.select(nodeId);nodes.importBundle(bundle.toString().getBytes(StandardCharsets.UTF_8));
        ...
        boolean multi=row.optBoolean("multihop_enabled",false);String entry=row.optString("multihop_entry_id",""),exit=row.optString("multihop_exit_id","");
        if(multi){if(entry.isEmpty()||exit.isEmpty()||entry.equals(exit)||findNode(entry)==null||findNode(exit)==null)throw new IllegalStateException("Saved multihop references missing/invalid Router nodes.");}
        String mode=normalizeMode(row.optString("mode","smart-auto"));List<String>layers=jsonStrings(row.optJSONArray("custom_layers"),32);if(mode.startsWith("custom:")&&!layers.isEmpty())upsertCustomPreset(mode.substring(7),layers);
        SharedPreferences.Editor edit=prefs().edit().putString(SELECTED_KIND,kind).putString(SELECTED_ID,nodeId).putString(MODE_KEY,mode)
                .putBoolean(MULTI_ON,multi).putString(MULTI_ENTRY,entry).putString(MULTI_EXIT,exit).putString(MULTI_MODE,row.optString("multihop_exit_mode","shadowsocks"));edit.apply();return toRecord(row);
    }
    ...
    private JSONObject snapshot(String name,String id) throws Exception {
        ...
        String mode=normalizeMode(p.getString(MODE_KEY,"smart-auto"));List<String>layers=customLayers(mode);boolean multi=p.getBoolean(MULTI_ON,false);String entry=p.getString(MULTI_ENTRY,""),exit=p.getString(MULTI_EXIT,"");
        if(multi&&(entry==null||exit==null||entry.isEmpty()||exit.isEmpty()||entry.equals(exit)||findNode(entry)==null||findNode(exit)==null))throw new IllegalStateException("Current multihop selection is incomplete or references missing nodes.");
        JSONObject row=new JSONObject().put("id",id).put("name",name).put("node_kind",kind).put("node_id",nodeId).put("mode",mode).put("custom_layers",new JSONArray(layers))
                .put("multihop_enabled",multi).put("multihop_entry_id",entry==null?"":entry).put("multihop_exit_id",exit==null?"":exit).put("multihop_exit_mode",p.getString(MULTI_MODE,"shadowsocks"));if(policy!=null)row.put("policy",policy);return row;
    }

    private void requireIdle(String action){AndroidHomeStateStore.Snapshot home=AndroidHomeStateStore.snapshot(context);AndroidRuntimeRegistry e=AndroidRuntimeRegistry.get(context);boolean busy=home.connected||"connecting".equals(home.phase)||e.orchestrator.isRunning()||e.multihop.isActiveOrTransitioning()||"UP".equals(e.singBox.getState())||"STARTING".equals(e.singBox.getState())||"STOPPING".equals(e.singBox.getState())||"UP".equals(e.xray.getState())||"STARTING".equals(e.xray.getState())||"STOPPING".equals(e.xray.getState());if(busy)throw new IllegalStateException("Disconnect Router VPN before "+action+"; live session identity and proof must remain immutable.");}
    ...
    private JSONArray readRows() throws Exception {if(!file.exists())return new JSONArray();if(!file.isFile()||file.length()<=0||file.length()>MAX_STORE)throw new IllegalStateException("Connection profile store is invalid or too large.");JSONObject root=new JSONObject(new String(readLimited(file,MAX_STORE),StandardCharsets.UTF_8));if(root.optInt("schema_version",0)!=SCHEMA_VERSION)throw new IllegalStateException("Unsupported connection profile store schema.");JSONArray rows=root.optJSONArray("profiles");if(rows==null)rows=new JSONArray();if(rows.length()>MAX_PROFILES)throw new IllegalStateException("Too many saved connection profiles.");Set<String>ids=new HashSet<>();for(int i=0;i<rows.length();i++){JSONObject row=rows.getJSONObject(i);String id=row.optString("id","");cleanName(row.optString("name",""));if(!safeId(id)||!ids.add(id))throw new IllegalStateException("Connection profile store contains invalid/duplicate ids.");String kind=row.optString("node_kind","");if(!"router-vpn".equals(kind)&&!"external".equals(kind))throw new IllegalStateException("Connection profile node kind is invalid.");if(row.optString("node_id","").isEmpty())throw new IllegalStateException("Connection profile node id is missing.");normalizeMode(row.optString("mode","smart-auto"));jsonStrings(row.optJSONArray("custom_layers"),32);JSONObject policy=row.optJSONObject("policy");if(policy!=null)for(String key:JSONObject.getNames(policy)==null?new String[0]:JSONObject.getNames(policy))if(!allowedPolicyKey(key))throw new IllegalStateException("Connection profile contains non-whitelisted node data: "+key);}return rows;}
    ...
    private void upsertCustomPreset(String name,List<String>layers)throws Exception{if(name==null||name.trim().isEmpty()||name.length()>64)return;JSONArray all=new JSONArray(prefs().getString(CUSTOM_KEY,"[]")),next=new JSONArray();for(int i=0;i<all.length();i++){JSONObject p=all.optJSONObject(i);if(p!=null&&!name.equals(p.optString("name","")))next.put(p);}next.put(new JSONObject().put("name",name).put("layers",new JSONArray(layers)));prefs().edit().putString(CUSTOM_KEY,next.toString()).apply();}
    ...
    private static Record toRecord(JSONObject row){return new Record(row.optString("id",""),row.optString("name",""),row.optString("node_kind",""),row.optString("node_id",""),row.optString("mode","smart-auto"),jsonStrings(row.optJSONArray("custom_layers"),32));}
