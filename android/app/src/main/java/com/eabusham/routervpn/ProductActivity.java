...
public final class ProductActivity extends Activity {
    ...
    private View buildUi(){
        ...
        statusView=text("Disconnected",13,false);statusView.setTextColor(Color.rgb(173,190,214));statusView.setTextIsSelectable(true);sheet.addView(statusView,margins(0,0,0,dp(7)));
    ...
    private void showHelp(){new AlertDialog.Builder(this).setTitle("Router VPN").setMessage("Map-first daily app: choose one Router or Custom node, Connect, optionally configure real multihop, then adjust Settings, Mode and DNS from the swipe-up control sheet. Connected means the selected native path passed the required proof; unsupported graphs remain unavailable.").setPositiveButton("Run onboarding again",(d,w)->AndroidProductOnboarding.show(this,true)).setNeutralButton("Android VPN settings",(d,w)->startActivity(new Intent(Settings.ACTION_VPN_SETTINGS))).setNegativeButton("Close",null).show();}
