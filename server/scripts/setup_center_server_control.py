...
(()=>{
 ...
 window.routerVpnStopServer=async()=>{if(!confirm('Stop Router VPN transport ingress? Setup Center/admin/recovery will remain reachable and profiles/keys will be preserved.'))return;try{render(await api('/api/admin/server-control/stop',{method:'POST'}))}catch(e){alert('Stop VPN failed: '+e.message)}};
 window.routerVpnEmergencyStopServer=async()=>{if(!confirm('Emergency Stop Router VPN? This immediately blocks transport ingress and removes currently live WireGuard-family peers. Profiles/keys remain stored for recovery.'))return;try{const d=await api('/api/admin/server-control/emergency-stop',{method:'POST'});await window.refreshRouterVpnServerControl();if(Number(d.live_peers_removed||0)>0)alert(`Emergency Stop removed ${d.live_peers_removed} live WireGuard-family peer(s).`)}catch(e){alert('Emergency Stop failed: '+e.message)}};
