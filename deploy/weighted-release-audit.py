...
GATES = [
    ...
    Gate("Linux native GTK amd64+ARM packages", 6.0, lambda: all_markers("client/linux/build-native-app.sh", 'SRC="$ROOT/client/linux/routervpn-gtk-product-v5.c"', 'SHIPPED=("$SRC"', '"$V4" "$V3" "$CORE")', "gcc -O2 -Wall -Wextra -Werror") and all_markers("client/linux/routervpn-gtk-product-v5.c", "gtk_window_new", "gtk_notebook_new", "--self-test") and all_markers("client/linux/routervpn-gtk-product.c", "http://127.0.0.1:8788", "/api/connect-logical", "/api/emergency-stop") and all(none_markers(rel, "WebKit", "WebView", "xdg-open", "sensible-browser") for rel in ("client/linux/routervpn-gtk-product-v5.c", "client/linux/routervpn-gtk-product-v4.c", "client/linux/routervpn-gtk-product-v3.c", "client/linux/routervpn-gtk-product.c")) and all_markers(".github/workflows/linux-native-app.yml", "ubuntu-24.04", "ubuntu-24.04-arm")),
