...
GATES = [
    ...
    Gate("iOS exact node proof, Libbox and strict unsupported fail-closed", 3.0, lambda: all_markers("ios/RouterVPN/PacketTunnel/PacketTunnelProvider.swift", 'body["node_id"] as? String == expectedNodeID', "strict Apple kill switch requested", "includeAllNetworks", "enforceRoutes", 'case "libbox":', 'case "external-libbox":', "RouterVPNLibboxEngine", "proveExternalExit") and all_markers("ios/RouterVPN/App/IOSRuntimeSelection.swift", 'case libbox = "libbox"', "sing-box.json", "Unimplemented helper/PQ/MAX chains and unsupported multihop combinations remain unavailable instead of faking Connected; native REALITY, PQ REALITY, XHTTP and dual-transport Xray graphs are supported.") and all_markers("ios/RouterVPN/App/RouterVPNModelExternal.swift", "external-libbox", "External OpenVPN — native pinned client with exact public-exit proof", "exact public-exit proof") and all_markers("ios/RouterVPN/PacketTunnel/RouterVPNExternalExit.swift", "LibboxRouterOpenVPNEndpoint", 'endpoint["type"] as? String == "openvpn-client"', 'endpoint["system"] as? Bool == false') and all_markers("mobile/routervpn_openvpn_native_test.go.tmpl", "verified-TLS-payload-and-shutdown", "untrusted-CA-rejected", "wrong-peer-name-rejected", "malformed-CA-rejected") and all_markers("ios/RouterVPN/project.yml", "test_mobile_openvpn_pinned.sh")),
    ...
    Gate("Linux native GTK amd64+ARM packages", 6.0, lambda: all_markers("client/linux/build-native-app.sh", 'SRC="$ROOT/client/linux/routervpn-gtk-product-v5.c"', 'START_LAYER_SETTINGS="$ROOT/client/linux/apply-start-layer-settings.py"', 'SHIPPED=("$SRC"', '"$V4" "$V3" "$CORE" "$START_LAYER_SETTINGS")', '"$SETTINGS_INC" "$MUTATION_SETTINGS"', 'python3 "$START_LAYER_SETTINGS" "$MUTATION_SETTINGS" "$HARDENED_SETTINGS"', "gcc -O2 -Wall -Wextra -Werror") and all_markers("client/linux/routervpn-gtk-product-v5.c", "gtk_window_new", "gtk_notebook_new", "--self-test") and all_markers("client/linux/routervpn-gtk-product.c", "http://127.0.0.1:8788", "/api/connect-logical", "/api/emergency-stop") and all(none_markers(rel, "WebKit", "WebView", "xdg-open", "sensible-browser") for rel in ("client/linux/routervpn-gtk-product-v5.c", "client/linux/routervpn-gtk-product-v4.c", "client/linux/routervpn-gtk-product-v3.c", "client/linux/routervpn-gtk-product.c")) and all_markers(".github/workflows/linux-native-app.yml", "ubuntu-24.04", "ubuntu-24.04-arm")),
