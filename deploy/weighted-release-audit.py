...
GATES = [
    ...
    Gate("iOS exact node proof and strict unsupported fail-closed", 3.0, lambda: all_markers("ios/RouterVPN/PacketTunnel/PacketTunnelProvider.swift", 'body["node_id"] as? String == expectedNodeID', "strict Apple kill switch requested", "includeAllNetworks", "enforceRoutes", "AmneziaWG, layered, ALL/MAX and multihop remain unavailable")),
...
def main() -> int:
    ...
    for gate in GATES:
        ...
        if ok:
            earned += g.weight
        if g.kind == "source":
            source_total += g.weight
            if ok:
                source_earned += g.weight
        rows.append({"name": g.name, "weight": g.weight, "kind": g.kind, "pass": ok, "note": g.note})
    result = {
