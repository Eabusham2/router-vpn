...
need("modes/kill-switch.py","policy drop",'action == "reassert"',"current profile policy is no longer always","use force-off recovery locally","if force:","remove_table()");need("modes/test_kill_switch.py","router-vpn-killswitch-reassert-","cannot read routers.json","kill_switch_policy':'always");need("client/install-linux.sh","nftables","Before=network-pre.target","RequiredBy=network-pre.target","kill-switch.py reassert","router-vpn-killswitch-recovery","force-off")
