...
need("modes/kill-switch.py","policy drop",'action == "reassert"',"current profile policy is no longer always","use force-off recovery locally","if force:","remove_table()");need("modes/test_kill_switch.py","router-vpn-killswitch-reassert-","persistent always","cannot read routers.json");need("client/install-linux.sh","nftables","Before=network-pre.target","RequiredBy=network-pre.target","kill-switch.py reassert","router-vpn-killswitch-recovery","kill-switch.py\" force-off")
...
need("client/native-wireguard-windows.ps1","WireGuard\\wireguard.exe","/installtunnelservice","/uninstalltunnelservice","Is-Administrator","Unsafe WireGuard profile path","will not fake native readiness through WSL","windows-kill-switch.ps1","Invoke-KillSwitch 'prepare'","Invoke-KillSwitch 'release'");need("client/native-windows-mode.ps1","sing-box.exe","xray.exe","hysteria2","shadowsocks","reality-vision","reality-pq-vision","split","max","Patch-SingBox","Get-SelectedProfile","fastest_dns_host","hijack-dns","HOMEVPN_JUMBO","9000","Write-Utf8NoBom","Native Windows TUN modes require an elevated Router VPN process","windows-kill-switch.ps1","Invoke-KillSwitch 'prepare'","Invoke-KillSwitch 'release'","Get-TunAlias");need("client/windows-kill-switch.ps1","Get-NetFirewallProfile -PolicyStore ActiveStore","Set-NetFirewallProfile","DefaultOutboundAction Block","New-NetFirewallRule","Remove-NetFirewallRule","original_profiles","ProgramData","Router VPN Kill Switch","InterfaceAlias","on-connect","always","force-off","literal IPv4/IPv6");need("client/test-windows-kill-switch.ps1","plan/rollback contract","hostname endpoint was accepted","default_outbound")
