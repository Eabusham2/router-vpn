...
final class ProductWindowController: NSWindowController, MKMapViewDelegate {
    ...
    func refreshAdvanced() { DispatchQueue.global(qos: .utility).async { [weak self] in guard let self else { return }; do { let data = try api.request("/api/multihop/status", timeout: 10); let pretty: String; if let j = try? JSONSerialization.jsonObject(with: data), let d = try? JSONSerialization.data(withJSONObject: j, options: [.prettyPrinted, .sortedKeys]) { pretty = String(data: d, encoding: .utf8) ?? "" } else { pretty = String(data: data, encoding: .utf8) ?? "" }; DispatchQueue.main.async { self.advancedText.string = "Multihop status (truthful platform readiness):\n\(pretty)\n\nStrict kill switch, LAN policy, MTU/Jumbo and selected DNS remain enforced by the runtime/controller; unsupported platform lifecycle states must fail closed." } } catch { DispatchQueue.main.async { self.advancedText.string = "Advanced status unavailable: \(error.localizedDescription)" } } } }
...
func runProductSelfTest() -> Int32 {
    let required = ["/api/status", "/api/profiles", "/api/logical-modes", "/api/auto", "/api/connect-logical", "/api/disconnect", "/api/profile/select", "/api/profile/latency", "/api/public-ip", "/api/dns/retest", "/api/emergency-stop", "/api/session/events", "/api/multihop/status", "/api/forward", "/api/forward/clear"]
    let tabs = ["Home", "Nodes & Map", "Modes", "DNS", "Advanced", "Forwarding", "Settings", "Help"]
    guard routerVPNProductBaseURL.absoluteString == "http://127.0.0.1:8788", routerVPNProductContractVersion == 2, required.count == 15, tabs.count == 8 else { return 2 }
