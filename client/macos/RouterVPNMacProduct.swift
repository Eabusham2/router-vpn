...
final class ProductWindowController: NSWindowController, MKMapViewDelegate {
    ...
    func advancedView() -> NSView {
        ...
        let note = NSTextField(wrappingLabelWithString: "Real Router VPN multihop path: client TUN → exit transport → entry WireGuard → exit node → Internet. External hops use the separate validated external-connect path in Nodes & Map."); note.textColor = .secondaryLabelColor; s.addArrangedSubview(note)
...
func runProductSelfTest() -> Int32 {
    let required = ["/api/status", "/api/profiles", "/api/logical-modes", "/api/auto", "/api/connect-logical", "/api/disconnect", "/api/profile/select", "/api/profile/latency", "/api/profile/pair", "/api/profile/import", "/api/external-profile/import", "/api/external-profile/connect", "entry_id", "/api/profile/delete", "/api/public-ip", "/api/dns/retest", "/api/emergency-stop", "/api/session/events", "/api/multihop/status", "/api/multihop/connect", "/api/forward", "/api/forward/clear"]
    let tabs = ["Home", "Nodes & Map", "Modes", "DNS", "Advanced", "Forwarding", "Settings", "Help"]
    guard routerVPNProductBaseURL.absoluteString == "http://127.0.0.1:8788", routerVPNProductContractVersion == 2, required.count == 22, tabs.count == 8 else { return 2 }
