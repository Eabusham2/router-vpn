...
SHIPPED=("$SRC" "$ONBOARDING_INC" "$HOME_INC" "$SETTINGS_INC" "$UNIFIED_INC" "$TELEMETRY_INC" "$V4" "$V3" "$CORE")
BUILD_DIR=$(mktemp -d "${TMPDIR:-/tmp}/router-vpn-linux-v9.XXXXXX")
EMBEDDED_V4="$BUILD_DIR/routervpn-gtk-product-v4-embedded.c"
BUILD_SRC="$BUILD_DIR/routervpn-gtk-product-v9-shipping.c"
...
for source in "$ONBOARDING_INC" "$HOME_INC" "$SETTINGS_INC" "$UNIFIED_INC" "$TELEMETRY_INC"; do
...
# Shipping v9 keeps the mature controller/product code, complete onboarding,
# truthful Home state and profile settings, renames the old tab-first builder,
# installs the unified map-first shell and then wires live native telemetry.
...
if text.count(install_old) != 1:
    raise SystemExit('Linux telemetry installer seam drifted')
...
for marker in (
    ...
    '#include "routervpn-profile-settings-v1.inc"', '#include "routervpn-unified-shell-v8.inc"', '#include "routervpn-telemetry-v9.inc"',
    ...
    'static void build_ui_legacy_v5(App *app) {', 'linux_install_telemetry_v9(&app);',
):
...
for marker in '#include "routervpn-product-onboarding-v6.inc"' '#include "routervpn-home-summary-v1.inc"' '#include "routervpn-profile-settings-v1.inc"' '#include "routervpn-unified-shell-v8.inc"' '#include "routervpn-telemetry-v9.inc"' 'gtk_assistant_set_current_page' 'onboarding_write_step_v6' 'refresh_home_summary_v6' 'Prove actual exit' 'G_CALLBACK(on_home_exit_v6)' 'Edit profile settings' 'G_CALLBACK(on_profile_settings_v7)' 'router-vpn-advanced-settings-v7' 'static void build_ui_legacy_v5(App *app) {' 'linux_install_telemetry_v9(&app);'; do grep -Fq "$marker" "$BUILD_SRC"; done
...
for marker in 'LinuxTelemetryV9' '⚡ Fastest' '/api/profile/fastest' '/api/connection/live-latency' '/api/multihop/live-latency' 'Performance' 'Throughput + Auto MTU' 'Forward'; do grep -Fq "$marker" "$TELEMETRY_INC"; done
...
echo "Built native Linux GTK Router VPN product with map-first unified shell, fastest-node connect, live path/multihop telemetry, truthful Home state, editable profile settings and resumable onboarding at $OUT"
