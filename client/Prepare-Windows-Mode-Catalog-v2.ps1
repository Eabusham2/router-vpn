...
$linuxModes = $null
try {
  $raw = & wsl.exe --exec wslpath -a -u $ModesDir 2>$null | Select-Object -First 1
  if ($raw) {
    $candidate = ([string]$raw).Trim()
    if ($candidate) { $linuxModes = $candidate }
  }
...
$layered = 'unavailable without WSL2'
if ($linuxModes) { $layered = 'WSL2' }
Write-Output "Prepared Windows mode catalog: raw WireGuard=native tunnel service; layered modes=$layered."
