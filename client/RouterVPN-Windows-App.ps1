...
$OnboardingSteps=@(
    ...
    @{Title='Tor censorship bridges';Body='Tor bridge profiles can use obfs4, meek, Snowflake, WebTunnel, or validated Auto / Custom bridge-line sets. Windows x64 can connect through the pinned native Tor Expert Bundle + Lyrebird full-device runtime when the capability check passes. Windows ARM64 remains unavailable until Tor Project provides a pinned native ARM64 Expert Bundle. Unsupported controls must show that exact reason instead of pretending to connect.'},
...
try{
    if($SelfTest){
        ...
        # Controller routes and helper-internal telemetry/Speed-Lab fields are
