...
$SpeedLabHelpers=Join-Path $PSScriptRoot 'RouterVPN-Windows-SpeedLab.ps1'
...
try{
    if($SelfTest){
        ...
        # Controller routes are already enforced across the composed sources above.
        # This second list is intentionally UI/product-only so a route living in a
        # telemetry or Tor component is not falsely required to be duplicated in
        # Product-v2 itself.
        foreach($marker in @('UnifiedShell','UnifiedMapCanvas','UnifiedConnectButton','UnifiedFastestNode','UnifiedLiveLatency','UnifiedForwardButton','UnifiedKillSwitch','UnifiedMultihop','UnifiedMultihopLatency','UnifiedPerformanceButton','UnifiedModeCombo','UnifiedDnsCombo','UnifiedTorButton','Tor bridges','ShowUnifiedTorBridgeBuilder','SMART AUTO','New CUSTOM preset','download_loaded_ms','upload_loaded_ms','bufferbloat','Auto timing','Custom timing','Snowflake','WebTunnel','Auto / Custom','System.Collections.Generic.HashSet','real stored coordinates')){if(-not$ProductSource.Contains($marker)){throw "Windows unified product self-test missing $marker"}}
