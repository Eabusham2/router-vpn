...
try{
    if($SelfTest){
        ...
        foreach($marker in @('UnifiedShell','UnifiedMapCanvas','UnifiedConnectButton','UnifiedKillSwitch','UnifiedMultihop','UnifiedModeCombo','UnifiedDnsCombo','SMART AUTO','New CUSTOM preset','/api/strategy/auto','/api/strategy/smart-auto','/api/strategy/custom','/api/connect-logical','/api/multihop/connect','/api/mtu/retest','system','real coordinates')){if(-not$ProductSource.Contains($marker)){throw "Windows unified product self-test missing $marker"}}
