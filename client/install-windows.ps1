...
if (-not (Test-Path "$Bundle\client.json")) { throw 'Run this from the extracted router-vpn-client-bundle folder.' }
$Root = "$env:ProgramData\RouterVPN"
New-Item -Force -ItemType Directory $Root | Out-Null
Copy-Item "$Bundle\client.json","$Bundle\modes.json" $Root -Force
