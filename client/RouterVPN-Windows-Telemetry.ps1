...
function Add-RouterVPNTelemetryWindowsShell {
    ...
    # Replace the legacy flat lat/lon grid with a native offline VPN globe.
    # Coordinates still come only from linked profile data; no IP geolocation or
    # fabricated device position is introduced. Nodes are clickable and live
    # multihop roles drive the colored route and packet animation.
    $globe=@'
$script:UnifiedRoute=$null
$script:UnifiedRoutePacket=$null
$script:UnifiedMapPhase=0.0
$script:UnifiedRoutePathMs=0.0
function GlobeXY([double]$Lat,[double]$Lon,[double]$Width,[double]$Height){$cx=$Width/2;$cy=$Height/2;$rx=[Math]::Max(40,$Width/2-18);$ry=[Math]::Max(40,$Height/2-16);$latRad=$Lat*[Math]::PI/180.0;$x=$cx+($Lon/180.0)*$rx*0.92*[Math]::Cos($latRad*0.55);$y=$cy-($Lat/90.0)*$ry*0.86;return @($x,$y)}
function AddGlobeLine($x1,$y1,$x2,$y2,[string]$Color,[double]$Thickness=1){$Line=New-Object Windows.Shapes.Line;$Line.X1=$x1;$Line.Y1=$y1;$Line.X2=$x2;$Line.Y2=$y2;$Line.Stroke=$Color;$Line.StrokeThickness=$Thickness;$Line.StrokeStartLineCap='Round';$Line.StrokeEndLineCap='Round';[void]$MapCanvas.Children.Add($Line);return $Line}
function DrawMap($Profiles,[string]$Selected){
 ...
 $Width=[Math]::Max(420,[double]$MapCanvas.ActualWidth);$Height=[Math]::Max(190,[double]$MapCanvas.ActualHeight);$cx=$Width/2;$cy=$Height/2;$rx=[Math]::Max(40,$Width/2-15);$ry=[Math]::Max(40,$Height/2-14)
 ...
 foreach($Lat in @(-60,-30,0,30,60)){$y=(GlobeXY $Lat 0 $Width $Height)[1];$half=$rx*[Math]::Cos(($Lat*[Math]::PI/180.0)*0.78)*0.92;[void](AddGlobeLine ($cx-$half) $y ($cx+$half) $y (if($Lat-eq0){'#436B98'}else{'#294866'}) (if($Lat-eq0){1.4}else{0.8}))}
 foreach($Lon in @(-120,-60,0,60,120)){$Arc=New-Object Windows.Shapes.Ellipse;$factor=[Math]::Max(.10,[Math]::Cos(($Lon*[Math]::PI/180.0)));$Arc.Width=[Math]::Max(8,$rx*2*$factor);$Arc.Height=$ry*2;$Arc.Stroke=if($Lon-eq0){'#436B98'}else{'#294866'};$Arc.StrokeThickness=if($Lon-eq0){1.3}else{0.8};$Arc.Fill='Transparent';[Windows.Controls.Canvas]::SetLeft($Arc,$cx-$Arc.Width/2);[Windows.Controls.Canvas]::SetTop($Arc,$cy-$ry);[void]$MapCanvas.Children.Add($Arc)}
 ...
 foreach($Profile in @($Profiles)){
  ...
  if($Lat-lt-90-or$Lat-gt90-or$Lon-lt-180-or$Lon-gt180-or($Lat-eq0-and$Lon-eq0)){continue}
  ...
  $Color=switch($Role){'entry'{'#3791FF'}'exit'{'#FF9A32'}'selected'{'#916CFF'}'external'{'#F25BAC'}default{'#48C7D6'}};$Size=if($Role-in@('entry','exit','selected')){16}else{12}
  $Dot=New-Object Windows.Shapes.Ellipse;$Dot.Width=$Size;$Dot.Height=$Size;$Dot.Fill=$Color;$Dot.Stroke='#EAF3FF';$Dot.StrokeThickness=1;$Dot.Tag=$ID;$Dot.Cursor='Hand';$Dot.ToolTip="$($Profile.name) • real stored coordinate $Lat,$Lon";[Windows.Controls.Canvas]::SetLeft($Dot,$X-$Size/2);[Windows.Controls.Canvas]::SetTop($Dot,$Y-$Size/2);$Dot.Add_MouseLeftButtonUp({param($sender,$event)try{[void](Api '/api/profile/select' 'POST' @{id=[string]$sender.Tag} 10);RefreshProduct;RefreshUnifiedTelemetry}catch{Log ('Map node select failed: '+$_.Exception.Message)}});[void]$MapCanvas.Children.Add($Dot)
  $Ms=if([double]$Profile.latency_trimmed_mean_ms-gt0){[double]$Profile.latency_trimmed_mean_ms}elseif([double]$Profile.latency_median_ms-gt0){[double]$Profile.latency_median_ms}else{0};$Label=New-Object Windows.Controls.TextBlock;$Label.Text=if($Ms-gt0){'{0}  {1:N1} ms'-f [string]$Profile.name,$Ms}else{[string]$Profile.name};$Label.Foreground='#E8F2FF';$Label.FontSize=11;$Label.Tag=$ID;$Label.Cursor='Hand';$Label.Add_MouseLeftButtonUp({param($sender,$event)try{[void](Api '/api/profile/select' 'POST' @{id=[string]$sender.Tag} 10);RefreshProduct;RefreshUnifiedTelemetry}catch{Log ('Map node select failed: '+$_.Exception.Message)}});[Windows.Controls.Canvas]::SetLeft($Label,$X+$Size/2+4);[Windows.Controls.Canvas]::SetTop($Label,$Y-8);[void]$MapCanvas.Children.Add($Label);$Count++
 }
 if($Entry-and$Exit-and$Points.ContainsKey($Entry)-and$Points.ContainsKey($Exit)){$a=$Points[$Entry];$b=$Points[$Exit];$Route=AddGlobeLine $a[0] $a[1] $b[0] $b[1] '#59A7FF' 4;$Route.Opacity=.88;$Packet=New-Object Windows.Shapes.Ellipse;$Packet.Width=8;$Packet.Height=8;$Packet.Fill='White';$Packet.Stroke='#8FD3FF';$Packet.StrokeThickness=2;[void]$MapCanvas.Children.Add($Packet);$script:UnifiedRoute=[pscustomobject]@{x1=[double]$a[0];y1=[double]$a[1];x2=[double]$b[0];y2=[double]$b[1]};$script:UnifiedRoutePacket=$Packet;$PathLabel=New-Object Windows.Controls.TextBlock;$PathLabel.Text=if($script:UnifiedRoutePathMs-gt0){'PATH {0:N1} ms'-f$script:UnifiedRoutePathMs}else{'LIVE MULTIHOP'};$PathLabel.Foreground='#CBE7FF';$PathLabel.FontSize=10;$PathLabel.FontFamily='Consolas';$PathLabel.Background='#AA0D1B31';$PathLabel.Padding='5,2';[Windows.Controls.Canvas]::SetLeft($PathLabel,(($a[0]+$b[0])/2)-28);[Windows.Controls.Canvas]::SetTop($PathLabel,(($a[1]+$b[1])/2)-22);[void]$MapCanvas.Children.Add($PathLabel)}
 if($Count-eq0){$Empty=New-Object Windows.Controls.TextBlock;$Empty.Text='No real node coordinates in linked profiles';$Empty.Foreground='#AEBBD5';[Windows.Controls.Canvas]::SetLeft($Empty,20);[Windows.Controls.Canvas]::SetTop($Empty,40);[void]$MapCanvas.Children.Add($Empty)}
...
'@
    ...
    # Smooth packet movement is independent of the 2-second controller refresh.
...
'@
    ...
    $ProductSource+="`n# Windows telemetry UX contract: stylized offline VPN globe, clickable real-coordinate nodes, role colors, measured node ms, live entry-to-exit route with animated packet/PATH ms, fastest-node connect dropdown, live path RTT beside Connect/Disconnect, live multihop IN/OUT/PATH RTT, authenticated real path Mbps, real routed hop Mbps when reachable, separate MTU optimizer, Performance panel, and real active-home forwarding master toggle.`n"
