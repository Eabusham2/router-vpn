...
function Get-GeneratedProfileFingerprint($Profile){
  ...
  $candidates=@();if($id){$candidates+=Join-Path (Join-Path (Join-Path $root 'generated') $id) $mode};$candidates+=Join-Path (Join-Path $root 'generated') $mode;$dir=$null;foreach($candidate in $candidates){$candidate=Resolve-RouterVPNPrivateChild $root $candidate;if(Test-Path -LiteralPath $candidate -PathType Container){$dir=Get-Item -LiteralPath $candidate;break}}
