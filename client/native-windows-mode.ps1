...
function Get-DnsSelection {
  ...
  if($env:HOMEVPN_DNS_MODE){$mode=$env:HOMEVPN_DNS_MODE.ToLowerInvariant()};if($env:HOMEVPN_DNS_PROTOCOL){$protocol=$env:HOMEVPN_DNS_PROTOCOL.ToLowerInvariant()};if($env:HOMEVPN_DNS_HOST){$dnsHost=$env:HOMEVPN_DNS_HOST};if($env:HOMEVPN_DNS_PORT-match'^\d+$'){$port=[int]$env:HOMEVPN_DNS_PORT};if($env:HOMEVPN_DNS_SERVER_NAME){$serverName=$env:HOMEVPN_DNS_SERVER_NAME};if($env:HOMEVPN_DNS_PATH){$path=$env:HOMEVPN_DNS_PATH}
  switch($mode){'home'{$dnsHost=if($env:HOMEVPN_ADGUARD4){$env:HOMEVPN_ADGUARD4}else{Profile-String $p 'adguard_ipv4' (Profile-String $p 'adguard_ipv6' '10.77.0.1')};$protocol='udp';$port=53;$serverName='';$path=''}'fastest'{$dnsHost=$fastest;$protocol='udp';$port=53;$serverName='';$path=''}'doh'{$protocol='https';if($port-le0){$port=443}}'dot'{$protocol='tls';if($port-le0){$port=853}}'doh3'{$protocol='h3';if($port-le0){$port=443}}'rescue'{$protocol='rescue';if([string]::IsNullOrWhiteSpace($dnsHost)){$dnsHost=$fastest};if($port-le0){$port=443}}default{if($protocol-eq'doh'){$protocol='https'}elseif($protocol-eq'dot'){$protocol='tls'}elseif($protocol-eq'doh3'){$protocol='h3'};if($port-le0){if($protocol-in@('https','h3')){$port=443}elseif($protocol-eq'tls'){$port=853}else{$port=53}}}}
  $serverName=Infer-DnsServerName $dnsHost $serverName;if($protocol-eq'rescue'){$protocol='https';if([string]::IsNullOrWhiteSpace($serverName)){$dnsHost='1.1.1.1';$serverName='cloudflare-dns.com';$port=443;$path='/dns-query'}}
  ...
  return[pscustomobject]@{mode=$mode;protocol=$protocol;host=$dnsHost.Trim('[]');port=$port;server_name=$serverName;path=$path}
}
function Patch-SingBox([string]$Path,[string]$Endpoint){
  ...
  if($env:HOMEVPN_SOCKS-eq'true'){$cfg.inbounds=@([pscustomobject]@{type='socks';tag='socks-in';listen='127.0.0.1';listen_port=1080;users=@()})}else{$mtu=0;if($env:HOMEVPN_JUMBO-eq'true'){$mtu=9000}elseif($env:HOMEVPN_MTU-match'^\d+$'){$mtu=[int]$env:HOMEVPN_MTU};if($mtu-gt0){foreach($inbound in @($cfg.inbounds)){if($inbound-and(Has-Property $inbound 'type')-and$inbound.type-eq'tun'){$inbound.mtu=$mtu}}}}
