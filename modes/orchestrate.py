...
def available(mode: dict) -> bool:
    if WANT_JUMBO and not mode.get("jumbo_supported", False): return False
    if WANT_DAITA and not mode.get("daita_supported", False): return False
...
def health() -> tuple[bool, float]:
    ...
    try:
        ...
        with urllib.request.urlopen(target, timeout=TEST_SECONDS) as r:
            if not (200 <= int(getattr(r, "status", 0)) < 300): return False, 0.0
            body = r.read(4096)
...
def stop_current(hold_kill_switch: bool = False) -> None:
    ...
    if current is not None and current.poll() is None:
        try:
            current.send_signal(signal.SIGINT); current.wait(timeout=3)
        except Exception:
            try: current.kill()
...
def smart_auto() -> int:
    best: dict | None = None; best_latency = 0.0; tested: list[str] = []
    for mode in modes:
        if not mode.get("auto_eligible") or not available(mode): continue
        tested.append(mode["id"]); ok, latency = launch(mode)
        if ok: best, best_latency = mode, latency; break
    if best is None:
        print("SMART AUTO: no working mode", file=sys.stderr); return 1
    visited = {best["id"]}
    while True:
        changed = False
        for candidate_id in best.get("smart_simplify", []):
            if candidate_id in visited or candidate_id not in by_id: continue
            visited.add(candidate_id); candidate = by_id[candidate_id]
            if not available(candidate): continue
            tested.append(candidate_id); last_good = best; last_latency = best_latency
            ok, latency = launch(candidate)
            if ok:
                print(f"SMART removed/replaced layers: {last_good['id']} -> {candidate_id}", flush=True)
                best, best_latency, changed = candidate, latency, True; break
            restored, restored_latency = launch(last_good)
            if not restored:
                print("SMART AUTO could not restore its last-known-good mode", file=sys.stderr); return 1
...
def custom() -> int:
    profile = selected_profile(); requested = []
    for raw in profile.get("custom_layers", []):
        value = str(raw).strip().lower()
        if value and value not in requested: requested.append(value)
    if not requested:
        print("CUSTOM: select at least one layer in the client first", file=sys.stderr); return 2
    preferred = str(profile.get("base_tunnel") or "wg").lower(); candidates = []
    for mode in modes:
        if mode.get("id") in {"smart-auto", "custom", "all"}: continue
        layers = [str(x).lower() for x in mode.get("layers", [])]
        if not layers or not all(x in layers for x in requested) or not available(mode): continue
        has_wg = "wireguard" in layers; has_awg = "amneziawg2" in layers
        if preferred.startswith("awg") or preferred.startswith("amnezia"): base_penalty = 0 if has_awg else 1 if has_wg else 0
        else: base_penalty = 0 if has_wg else 1 if has_awg else 0
    ...
    if not candidates:
        print("CUSTOM: no validated compatible stack contains that exact layer selection", file=sys.stderr); return 2
    for *_, mode in candidates:
        ok, latency = launch(mode)
        if ok:
            print("CUSTOM requested: " + ", ".join(requested), flush=True); return wait_selected(mode, latency)
    print("CUSTOM: matching stacks existed but none passed selected-node path proof", file=sys.stderr); return 1
