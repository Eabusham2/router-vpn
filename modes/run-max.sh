#!/usr/bin/env bash
set -euo pipefail
MODE=${1:?MAX mode};ROOT=${HOMEVPN_ROOT:-/opt/router-vpn-client};PROFILE_ID=$(printf '%s' "${HOMEVPN_PROFILE_ID:-router}"|tr -cd 'A-Za-z0-9_.-');PROFILE_ID=${PROFILE_ID:-router};ENDPOINT=${HOMEVPN_ENDPOINT:?Choose a router backend first};SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")"&&pwd);SOURCE="$ROOT/generated/$PROFILE_ID/$MODE";[[ -d "$SOURCE" ]]||SOURCE="$ROOT/generated/$MODE";RUN="$ROOT/run";CONF="$RUN/profile-$PROFILE_ID-$MODE";rm -rf "$CONF";mkdir -p "$RUN";cp -a "$SOURCE" "$CONF";export HOMEVPN_MODE="$MODE";export HOMEVPN_MTU=${HOMEVPN_MTU:-1280}
