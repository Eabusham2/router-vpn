...
python3 "$SCRIPT_DIR/start-layer.py" prepare "$ROOT" "$PROFILE_ID" "$MODE" "$CONF" "$ENDPOINT" >/dev/null
# shellcheck disable=SC2046
eval "$(python3 "$SCRIPT_DIR/start-layer.py" env "$CONF/start-layer.json")"
export HOMEVPN_START_LAYER HOMEVPN_START_LAYER_XOR HOMEVPN_START_LAYER_KEY_CONFIG HOMEVPN_START_LAYER_RELAY_LISTEN HOMEVPN_START_LAYER_RELAY_TARGET
...
start_layer_binary(){ if command -v router-vpn-start-layer-relay >/dev/null 2>&1; then command -v router-vpn-start-layer-relay; return; fi; if [[ -x "$ROOT/router-vpn-start-layer-relay" ]]; then printf '%s\n' "$ROOT/router-vpn-start-layer-relay"; return; fi; echo 'router-vpn-start-layer-relay is missing; reinstall the current client bundle' >&2; return 1; }
start_start_layer_relay(){
  [[ ${HOMEVPN_START_LAYER_XOR:-0} == 1 ]] || return 0
  [[ -n ${HOMEVPN_START_LAYER_RELAY_LISTEN:-} && -n ${HOMEVPN_START_LAYER_RELAY_TARGET:-} && -n ${HOMEVPN_START_LAYER_KEY_CONFIG:-} ]] || { echo 'AES+XOR start layer is missing relay ownership metadata' >&2; return 1; }
  local bin pid
  bin=$(start_layer_binary)
  start_bg "$bin" --mode client --listen "$HOMEVPN_START_LAYER_RELAY_LISTEN" --target "$HOMEVPN_START_LAYER_RELAY_TARGET" --key-config "$HOMEVPN_START_LAYER_KEY_CONFIG"
  pid=$LAST_BG_PID
  sleep 0.2
  kill -0 "$pid" >/dev/null 2>&1 || { echo 'AES ciphertext whitening relay failed to start' >&2; return 1; }
}
