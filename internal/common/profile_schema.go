...
func normalizeExternalNode(ext *ExternalNodeConfig) error {
	if ext == nil {
		return fmt.Errorf("external node requires protocol configuration")
	}
	...
	if blocks != 1 {
		return fmt.Errorf("external node requires exactly one protocol block")
	}
	switch ext.Protocol {
	case "wireguard":
		if ext.WireGuard == nil || ext.OpenVPN != nil || ext.Shadowsocks != nil || ext.SOCKS5 != nil {
			return fmt.Errorf("external wireguard node requires only the wireguard block")
		}
		w := ext.WireGuard
		w.PrivateKey = strings.TrimSpace(w.PrivateKey)
		w.PeerPublicKey = strings.TrimSpace(w.PeerPublicKey)
		w.PresharedKey = strings.TrimSpace(w.PresharedKey)
		w.Endpoint = strings.TrimSpace(w.Endpoint)
		if w.PrivateKey == "" || w.PeerPublicKey == "" || w.Endpoint == "" || len(w.Addresses) == 0 || len(w.AllowedIPs) == 0 {
			return fmt.Errorf("external wireguard requires private key, address, peer public key, endpoint and allowed IPs")
		}
	...
	case "openvpn":
		if ext.OpenVPN == nil || ext.WireGuard != nil || ext.Shadowsocks != nil || ext.SOCKS5 != nil {
			return fmt.Errorf("external openvpn node requires only the openvpn block")
		}
		o := ext.OpenVPN
		o.Config = strings.TrimSpace(o.Config)
	...
	case "shadowsocks":
		if ext.Shadowsocks == nil || ext.WireGuard != nil || ext.OpenVPN != nil || ext.SOCKS5 != nil {
			return fmt.Errorf("external shadowsocks node requires only the shadowsocks block")
		}
		s := ext.Shadowsocks
		s.Server = strings.TrimSpace(s.Server); s.Method = strings.TrimSpace(s.Method)
		if s.Server == "" || s.Port < 1 || s.Port > 65535 || s.Method == "" || s.Password == "" {
			return fmt.Errorf("external shadowsocks requires server, valid port, method and password")
		}
	case "socks5":
		if ext.SOCKS5 == nil || ext.WireGuard != nil || ext.OpenVPN != nil || ext.Shadowsocks != nil {
			return fmt.Errorf("external socks5 node requires only the socks5 block")
		}
		s := ext.SOCKS5
		s.Host = strings.TrimSpace(s.Host); s.Username = strings.TrimSpace(s.Username)
...
func NormalizeRouterProfile(p *RouterProfile) error {
	...
	p.NodeKind = strings.ToLower(strings.TrimSpace(p.NodeKind))
	if p.NodeKind == "" { p.NodeKind = "router-vpn" }
	switch p.NodeKind {
	case "router-vpn":
	...
	case "external":
		...
		// External custom nodes are data-plane peers, not privileged Router VPN
		// control planes. Refuse stale management credentials/proof metadata so a
		// converted profile cannot accidentally leak or misuse home-node secrets.
		if strings.TrimSpace(p.NodeProofID) != "" || strings.TrimSpace(p.RouterAPI) != "" || strings.TrimSpace(p.APIToken) != "" {
			return fmt.Errorf("external node cannot contain Router VPN proof/admin credentials")
		}
	default:
	...
	if p.KillSwitchPolicy == "" {
		if p.KillSwitch { p.KillSwitchPolicy = "on-connect" } else { p.KillSwitchPolicy = "off" }
	}
	...
	if p.StartupMode == "" { p.StartupMode = "manual" }
	...
	if p.MTUPolicy == "" { p.MTUPolicy = "default" }
	switch p.MTUPolicy {
	case "default", "auto":
		p.ManualMTU = 0
	case "manual":
		if p.ManualMTU < 576 || p.ManualMTU > 9000 { return fmt.Errorf("manual MTU %d is outside 576..9000", p.ManualMTU) }
	default:
		return fmt.Errorf("invalid MTU policy %q", p.MTUPolicy)
	}

	if p.DiagnosticsRetentionDays == 0 { p.DiagnosticsRetentionDays = 7 }
	if p.DiagnosticsRetentionDays < 1 || p.DiagnosticsRetentionDays > 365 { return fmt.Errorf("diagnostics retention must be between 1 and 365 days") }
	...
	if p.MultihopEnabled {
		if p.MultihopEntryID == "" || p.MultihopExitID == "" { return fmt.Errorf("multihop requires both an entry node and an exit node") }
		if p.MultihopEntryID == p.MultihopExitID { return fmt.Errorf("multihop entry and exit nodes must be different") }
	}
	return nil
}
...
func NormalizeRouterProfileStore(s *RouterProfileStore) error {
	...
	for i := range s.Profiles {
		if err := NormalizeRouterProfile(&s.Profiles[i]); err != nil { return fmt.Errorf("profile %q: %w", s.Profiles[i].ID, err) }
		id := strings.TrimSpace(s.Profiles[i].ID)
		if id == "" { return fmt.Errorf("profile id is empty") }
		if seen[id] { return fmt.Errorf("duplicate profile id %q", id) }
		seen[id] = true
	}
	if s.SelectedID == "" && len(s.Profiles) > 0 { s.SelectedID = s.Profiles[0].ID }
	if s.SelectedID != "" && !seen[s.SelectedID] { return fmt.Errorf("selected profile %q is not present", s.SelectedID) }
	return nil
}
...
func (p RouterProfile) MarshalJSON() ([]byte, error) {
	clone := p
	if err := NormalizeRouterProfile(&clone); err != nil { return nil, err }
	base, err := json.Marshal(routerProfileWire(clone)); if err != nil { return nil, err }
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(base, &raw); err != nil { return nil, err }
...
func (s RouterProfileStore) MarshalJSON() ([]byte, error) {
	clone := s
	if err := NormalizeRouterProfileStore(&clone); err != nil { return nil, err }
