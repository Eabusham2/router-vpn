...
// External protocol values are private node data. They are linked after the
// generic application is installed and must never be baked into public/generic
// installers or artifacts.
type ExternalWireGuardConfig struct {
	PrivateKey string `json:"private_key"`; Addresses []string `json:"addresses"`; PeerPublicKey string `json:"peer_public_key"`; PresharedKey string `json:"preshared_key,omitempty"`; Endpoint string `json:"endpoint"`; AllowedIPs []string `json:"allowed_ips"`; DNS []string `json:"dns,omitempty"`; MTU int `json:"mtu,omitempty"`
}
...
// accepted. ExpectedPublicIP is mandatory because a generic Internet success is
// not enough to prove that traffic actually exited through this custom node.
